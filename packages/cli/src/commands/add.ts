...
export async function addCommand(args: string[]): Promise<void> {
  ...
  if (!existsSync(vaspFile)) {
    log.error(`No main.vasp found in ${projectDir}. Run this command from your project root.`)
    process.exit(1)
  }
  ...
  switch (sub) {
    ...
    case 'page': {
      ...
      appendToVasp(vaspFile, source, block)
      log.success(`Added route '${routeName}' (${routePath}) and page '${pageName}' to main.vasp`)
    ...
    case 'crud': {
      ...
      const entityName = toPascal(name)
      if (ast.entities.length > 0 && !ast.entities.some((e) => e.name === entityName)) {
      ...
      if (ast.cruds.some((c) => c.entity === entityName)) {
        log.error(`A crud block for '${entityName}' already exists in main.vasp`)
        process.exit(1)
      }
      const block = [
        `\ncrud ${entityName} {`,
    ...
    case 'auth': {
      ...
      log.dim(`  You can add more auth methods (google, github) to the methods list`)
      break
    }
    ...
    case 'api': {
      ...
      const methodArg = (
        args.find((a) => a.startsWith('--method='))?.slice('--method='.length) ?? 'GET'
      ).toUpperCase()
      const pathArg = args.find((a) => a.startsWith('--path='))?.slice('--path='.length) ?? `/api/${toKebab(name)}`
      ...
      appendToVasp(vaspFile, source, block)
      log.success(`Added api '${apiName}' (${methodArg} ${pathArg}) to main.vasp`)
...
function appendToVasp(vaspFile: string, currentSource: string, block: string): void {
...
function buildFunctionStub(fnName: string, ext: string, kind: FunctionKind): string {
  if (ext === 'ts') {
    switch (kind) {
      case 'query':
