 ...
 */
function rewriteRuntimeImport(content: string, fileDir: string, runtimeDir: string): string | null {
...
export async function ejectCommand(args: string[] = []): Promise<void> {
  ...
  if (!confirm) {
    ...
    log.dim('  1. Copies @vasp-framework/runtime source into src/vasp/runtime/')
    log.dim('  2. Rewrites all imports from "@vasp-framework/runtime" to a local path')
    ...
    console.log()
    log.info('After ejecting, main.vasp is preserved but `vasp generate` will no longer work.')
  ...
  const runtimePkg = join(projectDir, 'node_modules', '@vasp-framework', 'runtime')
  ...
  for (const searchDir of searchDirs) {
    ...
    for (const filePath of files) {
      ...
      const rewritten = rewriteRuntimeImport(content, fileDir, targetRuntimeDir)
      if (rewritten !== null) {
  ...
  log.dim('  main.vasp is preserved for reference but `vasp generate` will no longer work.')
  log.dim('  Runtime composables are in src/vasp/runtime/ — feel free to customize them.')
}
