...
const _proc = (globalThis as any).process as { env?: Record<string, string>; stdout?: { isTTY?: boolean } } | undefined
...
function formatOne(d: ParseDiagnostic, lines: string[], filename: string): string {
  const header = clr(c.red + c.bold, `error[${d.code}]`) + clr(c.bold, `: ${d.message}`)
  ...
  const caret = ' '.repeat(caretCol) + clr(c.red + c.bold, '^'.repeat(caretLen))
