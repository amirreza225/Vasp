  ...
  run(): void {
    ...
    const entitiesWithSchema = ast.entities.map((entity) => {
      ...
      const selfRefArrayFields = entity.fields
        .filter((f) => f.isRelation && f.isArray && !f.isManyToMany && f.relatedEntity === entity.name)
        .map((f) => f.name)
      ...
      const manyToOne = entity.fields
        ...
        .map((f) => {
          ...
          const relationName = isSelfRef ? (selfRefArrayFields[0] ?? f.name) : undefined
          return {
            name: f.name,
      ...
      const singularSelfRefFields = entity.fields
        .filter((f) => f.isRelation && !f.isArray && f.relatedEntity === entity.name)
        .map((f) => f.name)

      const oneToMany = entity.fields
        .filter((f) => f.isRelation && f.isArray && !f.isManyToMany && entityNames.has(f.relatedEntity!))
        .map((f) => {
          const isSelfRef = f.relatedEntity === entity.name
          const relationName = isSelfRef ? (singularSelfRefFields[0] ?? f.name) : undefined
          return {
            fieldName: f.name,
            relatedEntity: f.relatedEntity!,
            relatedTable: `${toCamelCase(f.relatedEntity!)}s`,
      ...
      const manyToManyRefs = entity.fields
        .filter((f) => f.isManyToMany && f.isArray && f.isRelation && entityNames.has(f.relatedEntity!))
        .map((f) => ({
          fieldName: f.name,
      ...
      return {
        ...
        oneToMany,
        manyToManyRefs,
        hasRelations: manyToOne.length > 0 || oneToMany.length > 0 || manyToManyRefs.length > 0,
      }
    })
    ...
    const authUserHasRelations = authUserOneToMany.length > 0 || authUserManyToOne.length > 0 || authUserManyToManyRefs.length > 0
    ...
    const hasAnyRelations = entitiesWithSchema.some((e) => e.hasRelations) || authUserHasRelations || junctionTables.length > 0

    this.write(
      `drizzle/schema.${this.ctx.ext}`,
      this.render('shared/drizzle/schema.hbs', { entitiesWithSchema, crudsWithFields, hasAnyRelations, authUserExtraFields, authUserOneToMany, authUserManyToOne, authUserManyToOne, authUserManyToManyRefs, authUserHasRelations, enumDeclarations, hasEnums, junctionTables }),
    )
