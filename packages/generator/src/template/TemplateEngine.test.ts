...
describe("TemplateEngine helpers", () => {
  ...
  it("camelCase", () => {
    expect(
      engine.renderString("{{camelCase name}}", { name: "get-todos" }),
    ).toBe("getTodos");
    expect(
      engine.renderString("{{camelCase name}}", { name: "MyTodoApp" }),
    ).toBe("myTodoApp");
  });

  it("pascalCase", () => {
    expect(
      engine.renderString("{{pascalCase name}}", { name: "get-todos" }),
    ).toBe("GetTodos");
    expect(engine.renderString("{{pascalCase name}}", { name: "myApp" })).toBe(
      "MyApp",
    );
  });
  ...
  it("kebabCase", () => {
    expect(
      engine.renderString("{{kebabCase name}}", { name: "MyTodoApp" }),
    ).toBe("my-todo-app");
    expect(
      engine.renderString("{{kebabCase name}}", { name: "getTodos" }),
    ).toBe("get-todos");
  });

  it("join", () => {
    expect(
      engine.renderString('{{join arr ", "}}', { arr: ["a", "b", "c"] }),
    ).toBe("a, b, c");
  ...
  it("importPath rewrites .js to .ts when ext is ts", () => {
    expect(
      engine.renderString("{{importPath src ext}}", {
        src: "@src/queries.js",
        ext: "ts",
      }),
    ).toBe("@src/queries.ts");
    ...
    expect(
      engine.renderString("{{importPath src ext}}", {
        src: "@src/pages/Home.vue",
        ext: "ts",
      }),
    ).toBe("@src/pages/Home.vue");
  });

  it("eq helper", () => {
    expect(
      engine.renderString("{{#if (eq a b)}}yes{{else}}no{{/if}}", {
        a: "x",
        b: "x",
      }),
    ).toBe("yes");
  ...
  it("includes helper", () => {
    expect(
      engine.renderString("{{#if (includes arr item)}}yes{{else}}no{{/if}}", {
        arr: ["a", "b"],
        item: "b",
      }),
    ).toBe("yes");
  ...
  it("throws GeneratorError for unknown template key", () => {
    expect(() => engine.render("nonexistent.hbs", {})).toThrow(
      "Template not found",
    );
  });
});
...
describe("TemplateEngine — drizzleColumn helper (Phase 2)", () => {
  ...
  const render = (
    ...
    engine.renderString(
      "{{{drizzleColumn name type modifiers nullable defaultValue isUpdatedAt}}}",
      { name, type, modifiers, nullable, defaultValue, isUpdatedAt },
    );
...
describe("TemplateEngine — valibotSchema helper (Phase 4)", () => {
  ...
  const render = (
    ...
    engine.renderString("{{{valibotSchema type nullable optional}}}", {
      type,
      nullable,
      optional,
    });
  ...
  it("wraps nullable + optional DateTime with v.optional(v.union(...))", () => {
    const validDate = `v.pipe(v.string(), v.minLength(1), v.transform(s => new Date(s)), v.check(d => !isNaN(d.getTime()), 'Invalid date'))`;
    expect(render("DateTime", true, true)).toBe(
      `v.optional(v.union([v.null(), ${validDate}]))`,
    );
  });

  it("wraps nullable fields with v.nullable", () => {
    expect(render("String", true)).toBe(
  ...
  it("wraps nullable + optional fields with both wrappers", () => {
    expect(render("Boolean", true, true)).toBe(
...
describe("TemplateEngine — valibotSchema helper with validation rules", () => {
  ...
  const render = (
    ...
    engine.renderString(
      "{{{valibotSchema type nullable optional enumValues validation}}}",
      { type, nullable, optional, enumValues, validation },
    );
  ...
  it("generates both minLength and maxLength when both are set", () => {
    const result = render("String", false, false, undefined, {
      minLength: 3,
      maxLength: 30,
    });
    expect(result).toContain("v.minLength(3)");
