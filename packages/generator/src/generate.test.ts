...
`
  ...
  it('generates per-entity list view', () => {
    const ast = parse(ADMIN_VASP)
    generate(ast, { outputDir, templateDir: TEMPLATES_DIR, logLevel: 'silent', engine: sharedEngine })
    const todoView = readFileSync(join(outputDir, 'admin/src/views/todo/index.vue'), 'utf8')
  ...
  it('generates per-entity FormModal', () => {
    const ast = parse(ADMIN_VASP)
    generate(ast, { outputDir, templateDir: TEMPLATES_DIR, logLevel: 'silent', engine: sharedEngine })
    const modal = readFileSync(join(outputDir, 'admin/src/views/todo/FormModal.vue'), 'utf8')
  ...
  it('generates router with entity routes', () => {
    const ast = parse(ADMIN_VASP)
    generate(ast, { outputDir, templateDir: TEMPLATES_DIR, logLevel: 'silent', engine: sharedEngine })
    const router = readFileSync(join(outputDir, 'admin/src/router/index.js'), 'utf8')
  ...
  it('generates AdminLayout', () => {
    const ast = parse(ADMIN_VASP)
    generate(ast, { outputDir, templateDir: TEMPLATES_DIR, logLevel: 'silent', engine: sharedEngine })
    const layout = readFileSync(join(outputDir, 'admin/src/layouts/AdminLayout.vue'), 'utf8')
...
describe('CRUD list config — pagination, filtering, sorting, search', () => {
  ...
  it('generates page-based pagination when paginate: true', () => {
    ...
    const route = readFileSync(join(outputDir, 'server/routes/crud/task.js'), 'utf8')
