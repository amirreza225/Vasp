...
onMounted(async () => {
  ...
  try {
    ...
    // Label uses common display fields in priority order; falls back to the record's id.
    uploadedByOptions.value = (res?.items ?? []).map((r) => ({
      label: String(r.name ?? r.title ?? r.username ?? r.email ?? r.id),
      value: r.id,
    }))
  } catch { /* ignore — options stay empty */ }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.record?.id) {
      await AttachmentApi.update(props.record.id, payload)
    } else {
      await AttachmentApi.create(payload)
    }
    emit('saved')
  } catch (err) {
...
<template>
  <Dialog
    :visible="visible"
    :header="record?.id ? 'Edit Attachment' : 'Create Attachment'"
    :style="{ width: '560px' }"
    modal
  ...
  >
    <div class="vasp-admin-form">
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Filename</label>
        <InputText v-model="form.filename" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">MimeType</label>
        <InputText v-model="form.mimeType" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">SizeBytes</label>
        <InputNumber v-model="form.sizeBytes" :use-grouping="false" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Url</label>
      ...
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">UploadedBy</label>
        <Select
          v-model="form.uploadedById"
