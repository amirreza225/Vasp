...
async function fetchData() {
  loading.value = true
  try {
    const res = await PageVersionApi.list({
...
function confirmDelete(id) {
  confirm.require({
    ...
    accept: async () => {
      try {
        await PageVersionApi.remove(id)
    ...
    >
      <template #header>
        <div class="vasp-admin-table-header">
          <h2 class="vasp-admin-page-title">PageVersion Management</h2>
          <div class="vasp-admin-table-actions">
            ...
            <Button icon="pi pi-plus" label="Add PageVersion" @click="openCreate" />
          </div>
        </div>
      </template>
      ...
      <Column field="changeNote" header="ChangeNote" sortable style="min-width: 8rem">
      </Column>
