...
const nullableStringTextFields = ['changeNote', ]
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.record?.id) {
      await PageVersionApi.update(props.record.id, payload)
    } else {
      await PageVersionApi.create(payload)
    }
    emit('saved')
  } catch (err) {
...
<template>
  <Dialog
    :visible="visible"
    :header="record?.id ? 'Edit PageVersion' : 'Create PageVersion'"
    :style="{ width: '560px' }"
    modal
  ...
  >
    <div class="vasp-admin-form">
      ...
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">ChangeNote <span class="vasp-optional">(optional)</span></label>
        <InputText v-model="form.changeNote" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
