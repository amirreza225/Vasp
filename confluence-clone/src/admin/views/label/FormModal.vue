...
const nullableStringTextFields = ['color', ]
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.record?.id) {
      await LabelApi.update(props.record.id, payload)
    } else {
...
<template>
  <Dialog
    :visible="visible"
    :header="record?.id ? 'Edit Label' : 'Create Label'"
    :style="{ width: '560px' }"
    modal
  ...
  >
    <div class="vasp-admin-form">
      ...
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Color <span class="vasp-optional">(optional)</span></label>
