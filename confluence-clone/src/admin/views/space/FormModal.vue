...
const nullableStringTextFields = ['description', 'iconUrl', ]
...
onMounted(async () => {
  try {
    ...
    ownerOptions.value = (res?.items ?? []).map((r) => ({
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.record?.id) {
      await SpaceApi.update(props.record.id, payload)
    } else {
...
<template>
  <Dialog
    :visible="visible"
    :header="record?.id ? 'Edit Space' : 'Create Space'"
    :style="{ width: '560px' }"
    modal
  ...
  >
    <div class="vasp-admin-form">
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Name</label>
        <InputText v-model="form.name" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Key</label>
        <InputText v-model="form.key" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Description <span class="vasp-optional">(optional)</span></label>
        <Textarea v-model="form.description" :rows="4" class="w-full" auto-resize />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">IconUrl <span class="vasp-optional">(optional)</span></label>
        <InputText v-model="form.iconUrl" class="w-full" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">IsPublic</label>
        <ToggleSwitch v-model="form.isPublic" />
      </div>
      <div class="vasp-admin-form-field">
        <label class="vasp-admin-form-label">Owner</label>
        <Select
