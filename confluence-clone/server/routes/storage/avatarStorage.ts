...
import { MAX_BYTES, isMimeAllowed, saveFile } from '../../storage/avatarStorage.ts'
import { VaspError } from '../../middleware/errorHandler.ts'

export const avatarStorageUploadRoutes = new Elysia({ prefix: '/api/storage/avatarstorage' })
  /**
   * POST /api/storage/avatarStorage/upload
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      if (file.size > MAX_BYTES) {
        ...
        throw new VaspError(
          'PAYLOAD_TOO_LARGE',
          `File size ${file.size} exceeds the limit of 2mb`,
          413,
        )
      }
